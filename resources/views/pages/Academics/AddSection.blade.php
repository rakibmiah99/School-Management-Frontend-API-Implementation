...
@section('MainContent')
    ...
    <section class="section-examType--heading u-padding-lg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading--main">Add Section</h2>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-title">
                            <h3 class="heading--sub">Add Section</h3>
                        </div>
                        <div class="card-body">
                            <form id="CreateForm" class="mb-3">
                                <input
                                    ...
                                    required
                                />
                                <button type="submit" class="btn btn-text">✔ SAVE SECTION</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-title">
                            <h3 class="heading--sub">Section List</h3>
                        </div>
                        <div class="card-body">
                            <table class="table">
                                <thead>
...
@section('scripts-last')
    ...
    <script>
        ...
        createForm.on('submit', function (e) {
            ...
            axios.post('/academics/section/add', formData)
                .then(function (response) {
                    // PreloaderOFF();
                    if(response.status === 200 && response.data.status == true){
                        $('#CreateForm #exam-type-input').val("");
                        GetAll();
        ...
        updateForm.on('submit', function (e) {
            ...
            let formData = new FormData();
            formData.append('name', examType);
            axios.post('/academics/section/update/'+id, formData)
                .then(function (response) {
                    // PreloaderOFF();
                    if(response.status === 200 && response.data.status == true){
                        GetAll();
        ...
        table.on('click', '.delete', function () {
            ...
            axios.get('/academics/section/delete/'+id)
                .then(function (response) {
                    if(response.status === 200 && response.data.status == true){
                        GetAll();
                        Toast(response.data.message)
        ...
        table.on('click', '.edit', function () {
            ModalLoaderON();
            let id = $(this).attr('data-id');
            axios.get('/academics/section/get-single/'+id)
                .then(function (response) {
        ...
        GetAll();
        function GetAll() {
            axios.get('/academics/section/get-all')
                .then(function (response) {
                    PreloaderOFF();
                    if(response.status === 200){
                        table.empty();
