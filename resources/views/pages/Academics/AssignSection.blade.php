...
@section('MainContent')
    ...
    <section class="section-assignSection--heading u-padding-lg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading--main">Assign Section</h2>
                </div>
            </div>
        </div>
    </section>
                ...
                <section class="section-assignSection">
                    <div class="card">
                        <div class="card-title">
                            <h3 class="heading--sub">Assign Section</h3>
                        </div>
                        <div class="card-body">
                            <form id="CreateForm" action="">
                                ...
                                <select required id="selectSection" style="visibility: hidden;" multiple>
                ...
                <section class="section-assignSection--list">
                    <div class="card">
                        <div class="card-title d-flex justify-content-between me-5">
                            <h3 class="heading--sub">Assigned Section List</h3>
                        </div>
                        <div
...
@section('scripts-last')
    ...
    <script>
        ...
        Selectize(selectClassElem);
        Selectize(uSelectClassElem);
...
/*        selectClassElem.selectize({
        ...
        createForm.on('submit', function (e) {
            ...
            let url = "/academics/assign-section/add";
            let data = {
        ...
        updateForm.on('submit', function (e) {
            ...
            let url = `/academics/assign-section/update/${id}`;
            let data = {
                class_id: selectClass.val(),
                section_id: selectSection.val(),
            };
        ...
        $('#deleteModal').on('click','.btn-delete', function () {
           ...
           let id = $(this).attr('data-id');
           let url = `/academics/assign-section/delete/${id}`;
           axios.get(url)
            .then(function (response) {
                if(response.status === 200 && response.data.status == true){
                    Toast(response.data.message);
        ...
        table.on('click', '.edit', function () {
            ...
            axios.get(`/academics/assign-section/get-single/${id}`)
            .then(function (assign_section_response) {
                if(assign_section_response.status === 200){
                    $('#UpdateForm #selectClass').data('selectize').setValue(assign_section_response.data.class_id);
                    uSectionListElem.data('selectize').setValue(assign_section_response.data.section_id);
                    uSelectClassElem[0].selectize.disable();
                    ModalLoaderOFF();
                }
                else {
                    console.log(assign_section_response.data.section_id)
                }
                /*

                if(assign_section_response.status === 200){
                    $('#UpdateForm #selectClass').data('selectize').setValue(assign_section_response.data.class_id);
                    let classID = assign_section_response.data.class_id;
        ...
        GetAll();
        function GetAll() {
            axios.get('/academics/assign-section/get-all')
            .then(function (response) {
                PreloaderOFF();
                if(response.status === 200){
                    table.empty();
        ...
        function Item(table,item, index){
            var sections = "";
            var getSections = (item.section_name != null) ? item.section_name : [];
            ...
            table.append(`
                   <tr>
                        ...
                        <td>${sections.substr(0, sections.length-2)}</td>
                        <td>
                            <div class="d-flex">
                                <div>
                                    <button data-id="${item.class_id}"  type="button" class="btn edit px-2">
                                        <i
