...
@section('MainContent')
    ...
    <section class="section-attendanceReport--heading u-padding-lg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading--main">Exam Schedule Report</h2>
                </div>
            </div>
        </div>
    </section>
...
@section('scripts-last')
    ...
    <script>
        ...
        $('#csvBtn').on('click', function (e) {
            ...
            window.location.href = `/reports/exam-schedule/export-csv/${examType}/${startDate}/${endDate}${(classID != "") ? "/"+classID : "" }${(sectionID != "") ? "/"+sectionID : ""}`;
        });
        ...
        $('#pdfBtn').on('click', function (e) {
            ...
            let classID = $('#selectClass').val();
            let sectionID = $('#sectionList').val();
            window.location.href = `/reports/exam-schedule/export-pdf/${examType}/${startDate}/${endDate}${(classID != "") ? "/"+classID : "" }${(sectionID != "") ? "/"+sectionID : ""}`;
        });
        ...
        $('#filterBtn').on('click', function () {
            ...
            if(examType != ""){
                PreloaderON();
                axios.get(`/reports/exam-schedule/get/${examType}/${startDate}/${endDate}${(classID != "") ? "/"+classID : "" }${(sectionID != "") ? "/"+sectionID : ""}`)
                    .then(function (response) {
                        if(response.status === 200){
                            PreloaderOFF();
        ...
        function Item(item) {
            ...
            table.append(`
                <tr>
                    ...
                    <td>${(item.subject_data != null) ? item.subject_data.name : ""}</td>
                    <td>
                      ${item.total_marks}
                    </td>
                    <td>${(item.class_data != null) ? item.class_data.name : ""}</td>
                    <td>${(item.section_data != null) ? item.section_data.name : ""}</td>
                    <td>${(item.teacher_data != null) ? item.teacher_data.name : ""}</td>
                    <td>${(item.guard_data != null) ? item.guard_data.name : ""}</td>
                </tr>
            `)
        }
