...
@section('MainContent')
    ...
    <section class="section-addExpense--heading u-padding-lg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading--main">Add Income</h2>
                </div>
            </div>
        </div>
    </section>
                ...
                <section class="section-addExpense">
                    <div class="card">
                        <div class="card-title">
                            <h3 class="heading--sub">Add Income</h3>
                        </div>
                        <div class="card-body">
                            <form id="CreateForm" action="">
                                ...
                                <select id="selectType" class="w-100" required>
                                    <option class="dropdown-item" value="">Income Type</option>
                ...
                <section class="section-expense--list">
                    <div class="card">
                        <div class="card-title d-flex justify-content-between me-5">
                            <h3 class="heading--sub">Income List</h3>
                        </div>
...
@section('scripts-last')
    ...
    <script>
        ...
        createForm.on('submit', function (e) {
            ...
            formData.append('income_type', expense_type);
            formData.append('amount', amount);
            axios.post('/accounts/add-income/add', formData)
        ...
        updateForm.on('submit', function (e) {
            ...
            formData.append('income_type', expense_type);
            formData.append('amount', amount);
            axios.post('/accounts/add-income/update/'+id, formData)
                .then(function (response) {
                    // PreloaderOFF();
                    if(response.status === 200 && response.data.status == true){
                        GetAll();
        ...
        $('#deleteModal .btn-delete').on('click', function () {
            ...
            axios.get('/accounts/add-income/delete/'+id)
                .then(function (response) {
                    if(response.status === 200 && response.data.status == true){
                        GetAll();
                    }
        ...
        table.on('click', '.edit', function () {
            ModalLoaderON();
            let id = $(this).attr('data-id');
            axios.get('/accounts/add-income/get-single/'+id)
                .then(function (response) {
                    if(response.status === 200){
                        ...
                        $('#UpdateForm #selectType').data('selectize').setValue(data.income_data.id);
        ...
        GetAll();
        function GetAll() {
            axios.get('/accounts/add-income/get-all')
                .then(function (response) {
                    PreloaderOFF();
                    if(response.status === 200){
                        table.empty();
        ...
        function Item(table,item, index){
            table.append(`
                  <tr>
                        ...
                        <td>${(item.income_data != null) ? item.income_data.name : "" }</td>
                        <td>${item.amount}</td>
                        <td>
                            <div class="d-flex">
