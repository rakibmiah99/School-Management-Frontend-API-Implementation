...
@section('MainContent')
    ...
    <section class="section-uploadContent--heading u-padding-lg">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="heading--main">Approve Leave Request</h2>
                </div>
            </div>
        </div>
    </section>
                ...
                <section class="section-pendingLeave--list">
                    <div class="card">
                        <div class="card-title d-flex justify-content-between me-5">
                            <h3 class="heading--sub">Approved Leave Request List</h3>
                        </div>
...
@section('scripts-last')
    ...
    <script>
        ...
        $('#selectCriteria').on('change', function () {
            ...
            if(criteria != ""){
                let url = `/leave/Approved/get/${criteria}`;
                axios.get(url)
            ...
            table.append(`
                <tr>
                    ...
                    <td>
                        ...
                        <button data-id="${item.id}"  class="btn py-2 px-3 ${(item.status != 2)  ?  "btn-deny" : "disabled" } btn-deny bg-danger text-white" role="button">Denny</button>
            ...
            table.append(`
                <tr>
                    ...
                    <td>
                        ...
                        <button data-id="${item.id}"  class="btn ${(item.status != 2)  ?  "btn-deny" : "disabled" } py-2 px-3 bg-danger text-white " type="button">Denny</button>
        ...
        table.on('click', '.btn-deny', function () {
            ...
            axios.get(`/leave/deny/${criteria}/${id}`)
                .then(function (response) {
                    t.removeClass('d-none');
                    n.addClass('d-none');
                    if(response.status === 200 && response.data.status == true){
                        Toast("Canceled Leave");
